...
- **생산량 입력은 `예상 생산량 기준정보(#prodplan)` 탭에서만** 한다. 전력비 화면은 그 값을 **자동 반영(읽기 전용)**.
- **전력비 화면 입력(변수)**: 대상 월, 전력단가
- **실적 고정**: 전력사용량(kWh)
- 생산량(ProdPlan)이 바뀌면 → 전력원단위 → 전력비원단위가 **실시간 재계산**
...
생산량은 모든 원가 모듈(전력비, 추후 재료비/구매비 등)에 영향을 주는 **공통 입력값**이므로,
별도의 기준정보 탭에서 한 번만 입력하고 각 모듈이 이를 공유한다. (사용자 확정 접근 **1-A**)

### 핵심 설계
- **전역 상태 `ProdPlan`** (app.js): `ProdPlan.plan[month][line] = 예상생산량`
  - `ProdPlan.get(month, line)` → 입력값이 있으면 그 값, 없으면 **실적 생산량 fallback**
  - `localStorage` 영속화 (key: `precost_prodplan_v1`) — 새로고침해도 유지
  - `ProdPlan.bulkSet(rows)` → 엑셀 업로드 일괄 반영, `ProdPlan.clearMonth(month)` → 해당 월 초기화
- **각 모듈은 ProdPlan을 읽기만** 한다. 예: 전력비 `powerRender()`가 `ProdPlan.get()`으로 생산량을 주입.

### 화면 기능 (`#prodplan`)
- 대상 월 선택 → 호기별 표: **실적 생산량 / 예상 생산량(입력) / 증감%**
- 입력 즉시 저장(localStorage) + 증감%·KPI 실시간 갱신 (입력 포커스 유지)
- `이 달 전체 실적값으로` 버튼으로 월 단위 초기화
- KPI: 예상값 입력 호기 수 / 제지·화장지 예상 생산량(톤) / 실적 대비 변동%

### 엑셀 업로드/양식 (SheetJS, CDN `xlsx@0.18.5`)
- **양식 받기**: 호기 × 월(2026-01…12) 행렬을 실적값으로 채운 `.xlsx` 다운로드 (SheetJS 없으면 CSV fallback)
- **엑셀 업로드**: 첫 시트의 헤더(`호기`, 월 컬럼)를 자동 인식 → `ProdPlan.bulkSet`
  - 월 표기 자동 정규화: `2026-01`, `2026년 1월`, `2026.1`, `2026/01` 등
  - 결과 메시지: `N건 반영 / M건 건너뜀(호기·월 불일치)`
- 업로드 후 전력비 시뮬레이션에 **즉시 자동 반영**

### 연동 검증 (node)
- 제지3 2026-01 실적 원단위 **567.65** / 전력비원단위 **103.55** (PASS)
- ProdPlan으로 제지3 생산량 **+10%** → 원단위 **516.05** (=567.65/1.1, PASS) → 전력비 화면 자동 반영 확인
...
**Last Updated**: 2026-05-29 — 예상 생산량 기준정보 탭(1-A) + 엑셀 업로드 추가
