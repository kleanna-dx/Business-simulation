...
/* ── 대상 연/월 분리 선택 UI ──
   ...
   selMonth: 현재 선택된 "YYYY-MM"
   라벨에 실적/예상 표시. 데이터에 없는 연·월 조합은 비활성. */
function monthYears() {
  var ys = [];
  (PowerDB.months || []).forEach(function (m) { var y = m.split('-')[0]; if (ys.indexOf(y) < 0) ys.push(y); });
  return ys.sort();
}
function monthsOfYear(year) {
  return (PowerDB.months || []).filter(function (m) { return m.split('-')[0] === year; }).sort();
}
function buildYearMonthSelect(prefix, selMonth) {
  selMonth = selMonth || defaultMonth() || '';
  var selYear = selMonth ? selMonth.split('-')[0] : (monthYears()[0] || '');
  var selStyle = 'padding:9px 11px;border:1px solid var(--border-2);border-radius:8px;font-family:inherit';

  var yearOpts = monthYears().map(function (y) {
    return '<option value="' + y + '"' + (y === selYear ? ' selected' : '') + '>' + y + '년</option>';
  }).join('');

  var monthOpts = monthsOfYear(selYear).map(function (m) {
    var mm = m.split('-')[1];
    var label = (+mm) + '월' + (Period.isActual(m) ? ' · 실적' : ' · 예상');
    return '<option value="' + m + '"' + (m === selMonth ? ' selected' : '') + '>' + label + '</option>';
  }).join('');

  return '<div style="display:flex;gap:8px">'
    + '<select id="' + prefix + '_year" style="flex:1;' + selStyle + '">' + yearOpts + '</select>'
    + '<select id="' + prefix + '_month" style="flex:1.3;' + selStyle + '">' + monthOpts + '</select>'
    + '</div>';
}
// 연도 select 변경 시 월 select 옵션을 해당 연도로 재생성. 반환: 새로 선택된 "YYYY-MM"
function refreshMonthOptions(prefix, preferMonthNum) {
  var ySel = $('#' + prefix + '_year');
  var mSel = $('#' + prefix + '_month');
  if (!ySel || !mSel) return null;
  var year = ySel.value;
  var list = monthsOfYear(year);
  // 같은 월(MM)이 새 연도에도 있으면 유지, 없으면 첫 달
  var target = null;
  if (preferMonthNum) {
    target = list.filter(function (m) { return m.split('-')[1] === preferMonthNum; })[0] || null;
  }
  if (!target) target = list[0] || null;
  mSel.innerHTML = list.map(function (m) {
    var mm = m.split('-')[1];
    var label = (+mm) + '월' + (Period.isActual(m) ? ' · 실적' : ' · 예상');
    return '<option value="' + m + '"' + (m === target ? ' selected' : '') + '>' + label + '</option>';
  }).join('');
  return target;
}
...
PAGES.prodplan = function () {
  ...
  var head = '<div class="card"><div class="card__head"><h3><i class="fas fa-industry" style="color:var(--blue);margin-right:7px"></i>예상 생산량 기준정보 (이동계획)</h3>'
    ...
    + '<div class="field"><label style="display:block;font-size:12px;color:var(--muted);margin-bottom:5px">대상 연 / 대상 월</label>'
    + buildYearMonthSelect('pp', prodplanCurrentMonth()) + '</div>'
...
AFTER.prodplan = function () {
  ...
  var ys = $('#pp_year');
  if (ys) ys.addEventListener('change', function () {
    var prefNum = (ProdPlanState.month || defaultMonth() || '').split('-')[1];
    var target = refreshMonthOptions('pp', prefNum);
    if (target) { ProdPlanState.month = target; route(); }
  });
